...
if __name__ == "__main__":
    ...
    exp = "E1_final"
    record_csv = f"../../evaluation/metrics/evaluation_record/evaluation_record_{exp}.csv"
    record = pd.read_csv(record_csv, header=0)
    mc = MetricCollector()
    interval = 5
    wula_base_path = f"../../evaluation/metrics/wula/{exp}/"
    for i, r in record.iterrows():
        ...
        if r['wrkname'] != 'inferline':
            continue
        start_time_rc = r["start_time"]
        end_time_rc = r["end_time"]
        print(
        ...
        for csv_file in get_csv_files_in_bottom_directory(csv):
            ...
            wrkname = csv_file.split("/")[-4]
            prom_data_path = f"../../evaluation/metrics/system/{exp}/{wrkname}/{scheduler}/{start_time_rc}/"
            if not os.path.exists(prom_data_path):
                os.makedirs(prom_data_path)

            df = pd.read_csv(csv_file)
            start_time = start_time_rc - interval
            end_time = end_time_rc + interval
            if (not collect_record[(collect_record["uuid"] == evaluation_id)]["collected"].eq(1).any()):
                node_perf_all, gpu_perf_all = mc.collect_all_metrics(
                    start_time=start_time, end_time=end_time
                )
                gpu_perf_all["evaluation_id"] = evaluation_id
                gpu_perf_all.to_csv(
                    f"{prom_data_path}/../gpu_{evaluation_id}.csv",
                    header=False,
                )
                ...
                pod_metrics_all.to_csv(
                    f"{prom_data_path}/../pod_{evaluation_id}.csv",
                    header=False,
                )
                ...
                sum_metrics_all.to_csv(
                    f"{prom_data_path}/../sum_{evaluation_id}.csv",
                    header=False,
                )
