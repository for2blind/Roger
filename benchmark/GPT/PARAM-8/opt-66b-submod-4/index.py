...
with torch.inference_mode():
    model= torch.load('/data/model/openfaas/cdgp/subgraphs/opt-66b/PARAM_OAP/8/opt-66b-submod-4.pt', map_location=infer_device)
    # warm up 
    ...
    x = torch.load("/data/model/openfaas/cdgp/subgraphs/opt-66b/PARAM_OAP/8/opt-66b-inputs-4.pt", map_location=infer_device)
    output = model(*x)
...
async def call_next_function(start_time,uid):
    ...
    while True:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get("http://172.169.8.253:31112/function/opt-66b-submod-5-param-8.cdgp?uid="+str(uid), timeout=timeout) as response:
