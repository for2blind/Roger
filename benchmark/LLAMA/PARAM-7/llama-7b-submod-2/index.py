...
with torch.inference_mode():
    model= torch.load('/data/model/openfaas/cdgp/subgraphs/llama-7b/PARAM_OAP/7/llama-7b-submod-2.pt', map_location=infer_device)
    # warm up 
    ...
    x = torch.load("/data/model/openfaas/cdgp/subgraphs/llama-7b/PARAM_OAP/7/llama-7b-inputs-2.pt", map_location=infer_device)
    output = model(*x)
...
async def call_next_function(start_time,uid):
    ...
    while True:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get("http://172.169.8.253:31112/function/llama-7b-submod-3-param-7.cdgp?uid="+str(uid), timeout=timeout) as response:
...
async def ask_for_retry(retry_count,start_time,uid):
    async with aiohttp.ClientSession() as session:
        retry_request_data = {
            "model_name": f"'llama-7b-submod-2'",
            "stage": f"2",
            "uid":uid,
...
async def call_handler(path):
    try:
        ...
        return str({'result':f'infernece except: {e}','end_time':time.time(),'t_exec':time.time()-start_time,'stage':2}), 500
