...
with torch.inference_mode():
    model= torch.load('/data/model/openfaas/cdgp/subgraphs/llama-7b/PARAM_OAP/7/llama-7b-submod-6.pt', map_location=infer_device)
    # warm up 
    ...
    x = torch.load("/data/model/openfaas/cdgp/subgraphs/llama-7b/PARAM_OAP/7/llama-7b-inputs-6.pt", map_location=infer_device)
    output = model(*x)
...
async def ask_for_retry(retry_count,start_time,uid):
    async with aiohttp.ClientSession() as session:
        retry_request_data = {
            "model_name": f"'llama-7b-submod-6'",
            "stage": f"6",
            "uid":uid,
...
async def call_handler(path):
    try:
        ...
        await inference()
        return json.dumps({'result':'last','end_time':time.time(),'t_exec':time.time()-start_time,'stage':stage}), 200
