...
with torch.inference_mode():
    model= torch.load('/data/model/openfaas/cdgp/subgraphs/bert-21b/PARAM_OAP/8/bert-21b-submod-6.pt', map_location=infer_device)
    # warm up 
    ...
    x = torch.load("/data/model/openfaas/cdgp/subgraphs/bert-21b/PARAM_OAP/8/bert-21b-inputs-6.pt", map_location=infer_device)
    output = model(*x)
...
async def call_next_function(start_time,uid):
    ...
    while True:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get("http://172.169.8.253:31112/function/bert-21b-submod-7-param-8.cdgp?uid="+str(uid), timeout=timeout) as response:
...
async def ask_for_retry(retry_count,start_time,uid):
    async with aiohttp.ClientSession() as session:
        retry_request_data = {
            "model_name": f"'bert-21b-submod-6'",
            "stage": f"6",
            "uid":uid,
...
async def call_handler(path):
    try:
        ...
        return str({'result':f'infernece except: {e}','end_time':time.time(),'t_exec':time.time()-start_time,'stage':6}), 500
