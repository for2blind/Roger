...
with torch.inference_mode():
    model= torch.load('/data/model/openfaas/cdgp/subgraphs/bert-21b/PARAM_OAP/8/bert-21b-submod-2.pt', map_location=infer_device)
    # warm up 
    ...
    x = torch.load("/data/model/openfaas/cdgp/subgraphs/bert-21b/PARAM_OAP/8/bert-21b-inputs-2.pt", map_location=infer_device)
    output = model(*x)
...
async def call_next_function(start_time,uid):
    ...
    while True:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get("http://172.169.8.253:31112/function/bert-21b-submod-3-param-8.cdgp?uid="+str(uid), timeout=timeout) as response:
...
async def ask_for_retry(retry_count,start_time,uid):
    async with aiohttp.ClientSession() as session:
        retry_request_data = {
            "model_name": f"'bert-21b-submod-2'",
            "stage": f"2",
            "uid":uid,
