...
with torch.inference_mode():
    model= torch.load('/data/model/openfaas/cdgp/subgraphs/bert-21b/PARAM_OAP/8/bert-21b-submod-7.pt', map_location=infer_device)
    # warm up 
    ...
    x = torch.load("/data/model/openfaas/cdgp/subgraphs/bert-21b/PARAM_OAP/8/bert-21b-inputs-7.pt", map_location=infer_device)
    output = model(*x)
...
async def ask_for_retry(retry_count,start_time,uid):
    async with aiohttp.ClientSession() as session:
        retry_request_data = {
            "model_name": f"'bert-21b-submod-7'",
            "stage": f"7",
            "uid":uid,
...
async def call_handler(path):
    try:
        ...
        return str({'result':f'infernece except: {e}','end_time':time.time(),'t_exec':time.time()-start_time,'stage':7}), 500
